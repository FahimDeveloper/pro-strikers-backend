...
const createOrUpdateGeneralMembershipSubscription = async (payload: {
  email: string;
  plan: 'monthly' | 'quarterly';
...
const createOrUpdateAcademyMembershipSubscription = async (payload: {
  email: string;
  plan: 'monthly' | 'quarterly';
  membership: 'youth_training_membership';
  amount?: number;
}) => {
  const { email, membership, plan, amount } = payload;
...
export const reCurringProccess = async (body: Buffer, headers: any) => {
  ...
  let membershipType: 'general' | 'academy' | null = null;
